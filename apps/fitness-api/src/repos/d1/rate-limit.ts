...
import { rateLimits } from '@rallypoint/fitness-db'
import {
...
// this stays the shared D1 repo. fitness has no scheduled handler, so the D1
// branch keeps its existing opportunistic prune on window rollover
// (opportunisticPrune: true); the DO backend self-cleans via per-bucket
// alarms instead.
