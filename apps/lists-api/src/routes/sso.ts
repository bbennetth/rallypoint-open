...
// SSO + session-lifecycle routes (lists side of the §3.13 bootstrap,
// mirroring apps/events-api). All live under /api/v1/ui/* so origin +
...
export const ssoRoutes = new Hono<HonoApp>()
  ...
  .post('/api/v1/ui/sso/exchange', rateLimit({ route: 'sso-exchange', perIp: { limit: 10, windowSeconds: 10 * 60 } }), async (c) => {
    const env = c.var.env
    ...
    // before redirecting to RPID (anti-CSRF for the SSO leg). Clear the
    // state cookie either way — it is single-use.
    const stateCookieName = env.LISTS_SSO_STATE_COOKIE_NAME
    ...
    // Mint the lists bearer, seal the RPID bearer AAD-bound to the new
    // row's id_hash, and persist the session.
    const listsBearer = generateRawToken(LISTS_SESSION_BEARER_PREFIX)
    const idHash = hashToken(listsBearer)
    ...
    await c.var.repos.sessions.create({
      ...
      ipHash: hashIp(
        ...
        dailySalt(env.LISTS_SESSION_KEY_V1),
      ),
    ...
    c.header(
      'Set-Cookie',
      buildSetCookie(env.LISTS_SESSION_COOKIE_NAME, listsBearer, {
        maxAge,
        httpOnly: true,
  ...
  .post('/api/v1/ui/signout', async (c) => {
    const env = c.var.env
    const raw = readCookie(c.req.header('cookie') ?? '', env.LISTS_SESSION_COOKIE_NAME)
    if (raw) {
    ...
    c.header('Set-Cookie', buildClearCookie(env.LISTS_SESSION_COOKIE_NAME, true, env.NODE_ENV === 'production'))
    return c.body(null, 204)
  })
