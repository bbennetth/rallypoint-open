...
// SQLite via `npm run lists:db:generate`) and are applied with wrangler:
//   wrangler d1 migrations apply <DB> --env <qa|prod>      (deploy)
//   readD1Migrations(...) + applyD1Migrations(...)          (tests)
// The wrangler binding + apply step are wired with the lists-api Worker in
// Phase 4. Kept as a stub so the `lists:db:migrate` script path survives.

throw new Error(
  'packages/lists-db migrate runner was retired in the D1 migration (#313). ' +
