...
describe('realtime routes (memory)', () => {
  ...
  function headers(bearer: string): Record<string, string> {
    return {
      ...
      // methods. req() is used for ledger POSTs (setup); supplying the
      // configured origin here is safe for reads too.
      origin: env.MONEY_UI_ORIGIN,
    }
  }
  ...
  it('skips secureHeaders on the WS-upgrade route (other routes still carry them)', async () => {
    const token = mintToken('money:ledger:led_secure')
    const res = await app.request(
      `http://localhost/api/v1/ui/realtime?token=${encodeURIComponent(token)}`,
      { headers: { Upgrade: 'websocket' } },
    )
  ...
  it('forwards an immutable-headers DO response without a 500 (prod repro)', async () => {
    ...
    const token = mintToken('money:ledger:led_imm')
