...
// D1 integration tests for the money SSO + session-lifecycle routes.
// Replaces sso.it.test.ts.

const CSRF = 'csrf_token_value_aaaaaaaaaaaaaaaaaaaaaaaaaa'

const FAKE_USER = {
  userId: 'user_01HFAKEMONEYUSERID0000000',
  email: 'money-test@example.com',
  emailVerified: true,
  displayName: 'Money Tester',
  pictureUrl: null,
  username: 'moneytester',
...
// A minimal stubbed ObjectStore — session routes never touch it, but
// Services demands one. Throws on any accidental call.
const noopObjectStore: Services['objectStore'] = {
  async put() {
    throw new Error('sso tests should not call objectStore')
  },
  async get() {
    throw new Error('sso tests should not call objectStore')
  },
...
describe('D1 integration — money SSO + session surface', () => {
  let repos: Repos
  let envVars: Env
  ...
  // exchange → already_consumed (#272) and signout → upstream RPID
  // hiccup / no-cookie / stale-row (#217).
  ...
  // Mint a money session in the DB and return the raw bearer.
  ...
  function csrfCookiePair(): string {
    return `${envVars.MONEY_CSRF_COOKIE_NAME}=${CSRF}`
  }
  ...
  it('GET /session with a valid session cookie returns the user id', async () => {
    const userId = 'user_01HMONEYTEST00000000001'
  ...
  it('GET /session folds the shared settings doc in for the session user', async () => {
    const userId = 'user_01HMONEYTEST00000000010'
    const bearer = await loginAs(userId)
  ...
  it('GET /session degrades to an empty settings doc when the fold-in fails', async () => {
    const userId = 'user_01HMONEYTEST00000000011'
    const bearer = await loginAs(userId)
  ...
  it('GET /session folds the RPID profile in for the session user', async () => {
    const userId = 'user_01HMONEYTEST00000000030'
    const bearer = await loginAs(userId)
  ...
  it('GET /session degrades profile to null when the batch lookup throws', async () => {
    const userId = 'user_01HMONEYTEST00000000031'
    const bearer = await loginAs(userId)
  ...
  it('GET /settings/shared returns the namespace doc for the session user', async () => {
    const userId = 'user_01HMONEYTEST00000000012'
    const bearer = await loginAs(userId)
  ...
  it('GET /settings/money (own client namespace) is allowed', async () => {
    const userId = 'user_01HMONEYTEST00000000013'
    const bearer = await loginAs(userId)
    const res = await app.request('http://localhost/api/v1/ui/settings/money', {
      method: 'GET',
      headers: sessionHeaders(bearer),
    })
    expect(res.status).toBe(200)
    expect(settingsGet).toHaveBeenLastCalledWith(userId, 'money')
  })

  it('GET /settings/events (foreign namespace) is 403 without hitting RPID', async () => {
    const userId = 'user_01HMONEYTEST00000000014'
    const bearer = await loginAs(userId)
  ...
  it('PATCH /settings/shared forwards the patch and returns the merged doc', async () => {
    const userId = 'user_01HMONEYTEST00000000015'
    const bearer = await loginAs(userId)
  ...
  it('PATCH /settings/shared with a non-object body is 400', async () => {
    const userId = 'user_01HMONEYTEST00000000016'
    const bearer = await loginAs(userId)
  ...
  it('PATCH /settings/events (foreign namespace) is 403', async () => {
    const userId = 'user_01HMONEYTEST00000000017'
    const bearer = await loginAs(userId)
    settingsPatch.mockClear()
    const res = await app.request('http://localhost/api/v1/ui/settings/events', {
      method: 'PATCH',
  ...
  it('maps an RPID SettingsError onto the money error envelope', async () => {
    const userId = 'user_01HMONEYTEST00000000018'
    const bearer = await loginAs(userId)
  ...
  it('POST /sso/exchange stores a daily-salted ip_hash matching extractIp+hashIp', async () => {
    const knownIp = '192.0.2.99'
    const stateNonce = 'state_iphash_money'
    ...
    const sessionCookieEntry = setCookies.find((h) =>
      h.startsWith(envVars.MONEY_SESSION_COOKIE_NAME + '='),
    )
    expect(sessionCookieEntry).toBeTruthy()
    const bearerMatch = sessionCookieEntry!.match(
      new RegExp(`${envVars.MONEY_SESSION_COOKIE_NAME}=([^;]+)`),
    )
    ...
    const expectedIpHash = hashIp(
      ...
      dailySalt(envVars.MONEY_SESSION_KEY_V1),
    )
  ...
  it('POST /signout propagates the decrypted RPID bearer upstream, deletes the row, clears the cookie (#93)', async () => {
    ...
    const userId = 'user_01HMONEYTEST00000000020'
    const bearer = await loginAs(userId)
    ...
    expect(setCookie).toContain(envVars.MONEY_SESSION_COOKIE_NAME)
  ...
  it('POST /signout still succeeds locally when the upstream RPID signout fails (best-effort, #93)', async () => {
    ...
    const userId = 'user_01HMONEYTEST00000000021'
    const bearer = await loginAs(userId)
    const idHash = hashToken(bearer)
    ...
    expect(signoutRpidBearer).toHaveBeenCalledWith(userId)
    expect(await repos.sessions.findByIdHash(idHash)).toBeNull()
    expect(res.headers.get('set-cookie') ?? '').toContain(envVars.MONEY_SESSION_COOKIE_NAME)
  })
  ...
  it('POST /signout with an already-deleted session row is a 204 no-op (no upstream call)', async () => {
    signoutRpidBearer.mockClear()
    const userId = 'user_01HMONEYTEST00000000022'
    const bearer = await loginAs(userId)
    const idHash = hashToken(bearer)
