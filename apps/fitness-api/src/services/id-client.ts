...
// Delegates to the `Service<IdRPC>` binding (fitness's catch-up to
// feat/rpc-bindings). Replaces the legacy FITNESS_API_KEY + HTTP path,
...
export function createIdClientService(binding: Service<IdRPC>): IdClientService {
  return {
    ...
    async signoutRpidBearer(bearer) {
      try {
        await binding.signoutSession(bearer, { client: 'fitness' })
      } catch (err) {
        throw new Error('rpid_transport_error', { cause: err })
      }
    },
