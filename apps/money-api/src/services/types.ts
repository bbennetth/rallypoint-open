// External-service contracts. money-api reaches RPID over HTTP for
...
// The fields money-api keeps from an RPID SSO exchange.
...
export interface RpidSsoService {
  // POST RPID /api/v1/sdk/sso/exchange with the MONEY_API_KEY bearer.
...
export interface IdClientService {
  ...
  // `revoked` distinguishes a 401 (delete the money session) from a
...
export interface Services {
  ...
  // Generic per-user settings access over RPID's SDK. money-api uses
...
// Re-export so route handlers can name the ObjectStore type without
// pulling in the object-store package directly.
