...
// message "UNIQUE constraint failed: <table>.<col>", but drizzle's d1
// driver wraps it: the outer error's message is "Failed query: …" and
// the SQLite text lives on the `.cause` chain. So we walk message +
// causes rather than checking only the top-level message.
...
export function mapUniqueViolation(err: unknown): Error {
  ...
  while (cur && typeof cur === 'object' && !seen.has(cur)) {
    ...
    if (typeof msg === 'string' && msg.includes('UNIQUE constraint failed')) {
      // e.g. "UNIQUE constraint failed: list_shares.list_id, list_shares.user_id"
