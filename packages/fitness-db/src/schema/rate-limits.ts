...
// rate_limits — SQLite sliding-window counter store for fitness-api.
// Mirrors packages/money-db/src/schema/rate-limits.ts.
//
...
// the previous window. There is no TTL/`scheduled` reaper (fitness-api has
...
// capping each active bucket at its two live windows and bounding
...
export const rateLimits = sqliteTable(
  ...
  (t) => ({
    pk: primaryKey({
      ...
      name: 'fitness_rate_limits_pkey',
    }),
  }),
)

export type DbFitnessRateLimit = typeof rateLimits.$inferSelect
