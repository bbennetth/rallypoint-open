...
// We POST the code+state to events-api's exchange endpoint, which
// validates state against the cookie, swaps the code for an RPID
// session bearer, seals it, and sets our events session cookie.
//
...
function safeDest(raw: string | null): string {
  if (!raw) return '/me/events'
  try {
    const url = new URL(raw, window.location.origin)
    if (url.origin !== window.location.origin) return '/me/events'
        ...
        {phase.kind === 'exchanging' && (
          <>
            ...
            <p className="mono" style={{ color: 'var(--ink-dim)', fontSize: 13 }}>
              Connecting your Rallypoint ID to Events.
            </p>
          </>
        )}
        {phase.kind === 'error' && (
