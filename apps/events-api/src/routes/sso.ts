...
// SSO + session-lifecycle routes (events side of the §3.13 bootstrap). The
...
export const ssoRoutes = new Hono<HonoApp>()
  ...
  .post(
    ...
    rateLimit({ route: 'sso-exchange', onStoreError: 'deny', perIp: { limit: 10, windowSeconds: 600 } }),
    createSsoExchangeHandler({
      bearerPrefix: EVENTS_SESSION_BEARER_PREFIX,
      stateCookieEnvKey: 'EVENTS_SSO_STATE_COOKIE_NAME',
      sessionCookieEnvKey: 'EVENTS_SESSION_COOKIE_NAME',
      keyV1EnvKey: 'EVENTS_SESSION_KEY_V1',
      keyVersionEnvKey: 'EVENTS_SESSION_KEY_VERSION',
      encryptBearer,
      readJsonBody,
      errors: {
