...
// planner-side session resolution (events-v1 design §3.13, reused). The
// cookie carries the opaque planner bearer; we store sha256(bearer) as
...
export function requireSession(): MiddlewareHandler<HonoApp> {
  return async (c, next) => {
    const cookieName = c.var.env.PLANNER_SESSION_COOKIE_NAME
    const raw = readCookie(c.req.header('cookie') ?? '', cookieName)
    if (!raw) {
      throw errors.unauthorized()
    }
    if (!tokenHasPrefix(raw, PLANNER_SESSION_BEARER_PREFIX)) {
    ...
    try {
      bearer = decryptBearer({
        ...
        aad: idHash,
        env: { PLANNER_SESSION_KEY_V1: c.var.env.PLANNER_SESSION_KEY_V1 },
      })
    } catch {
