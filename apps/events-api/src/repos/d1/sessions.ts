...
// (see packages/events-db/src/schema/sessions.ts). We encode Buffers
// to base64 on write and decode back to Buffer on read, so the AES-GCM
// crypto layer (apps/events-api/src/crypto/encryption.ts) is unchanged.
// Mirrors apps/lists-api/src/repos/d1/sessions.ts.

function rowToSession(row: typeof sessions.$inferSelect): EventsSessionRecord {
  return {
    idHash: row.idHash,
    userId: row.userId,
...
export class D1EventsSessionRepo implements EventsSessionRepo {
  ...
  async deleteExpiredBefore(cutoff: Date): Promise<number> {
    const rows = await this.db
      .delete(sessions)
      .where(lte(sessions.absoluteExpiresAt, cutoff))
