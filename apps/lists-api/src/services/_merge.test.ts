...
import type { FieldDefDto, ListItemDto, SeriesDto } from './rpc-core.js'

// Pure unit coverage for the merge-decision helpers (#543, lifted from
// planner-api into lists-api). No I/O — these assert the branch logic
// mergeListsCore composes, against the rpc-core DTO shapes.

const ISO = '2026-01-01T00:00:00.000Z'

function def(
  over: Partial<FieldDefDto> & { id: string; label: string; fieldType: string },
): FieldDefDto {
  return {
    listId: 'lst_x',
    key: 'k',
...
function series(over: Partial<SeriesDto> & { id: string }): SeriesDto {
  return {
    listId: 'lst_src',
    title: 'S',
    notes: null,
...
describe('planFieldDefs', () => {
  it('reuses a target def with matching (label,type); creates the rest', () => {
    const sourceDefs = [
      def({ id: 'lfd_s1', label: 'Effort', fieldType: 'number' }),
      def({ id: 'lfd_s2', label: 'Tag', fieldType: 'text' }),
    ]
    const targetDefs = [def({ id: 'lfd_c1', label: 'effort', fieldType: 'number' })]
    const plan = planFieldDefs(sourceDefs, targetDefs)
...
describe('remapCustomFields', () => {
  it('translates source def-id keys to target ids', () => {
...
describe('itemCreateInput', () => {
  ...
  it('remaps source def-id keys to target ids (drops unmapped keys)', () => {
