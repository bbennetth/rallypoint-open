...
// actually does its job (verifySessionOnce rebuilds per-call and never
// caches — do not use it here).

export function createIdClientService(opts: {
  apiBase: string
  // Optional fetch override — a Cloudflare service-binding fetcher when one
  // is bound (see worker.ts / wrangler.toml RPID binding), else the global
  // fetch. Dispatches the RPID hop in-process instead of through the public
  // edge (which drops same-account loopback fetches). `| undefined` so the
  // caller can pass through an absent binding under exactOptionalPropertyTypes.
