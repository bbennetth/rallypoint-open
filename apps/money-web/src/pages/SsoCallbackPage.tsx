...
// We POST the code+state to money-api's exchange endpoint, which
// validates state against the cookie, swaps the code for an RPID
// session bearer, seals it, and sets our money session cookie.
...
function safeDest(raw: string | null): string {
  if (!raw) return '/me/ledgers'
  try {
    const url = new URL(raw, window.location.origin)
    if (url.origin !== window.location.origin) return '/me/ledgers'
        ...
        {phase.kind === 'exchanging' && (
          <>
            ...
            <p className="mono" style={{ color: 'var(--ink-dim)', fontSize: 13 }}>
              Connecting your Rallypoint ID to Money.
            </p>
          </>
        )}
        {phase.kind === 'error' && (
