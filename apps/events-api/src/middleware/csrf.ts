...
// CSRF double-submit, ported from RPID. GET /api/v1/ui/csrf issues a
// random token in a JS-readable cookie + body; state-changing
// requests under /api/v1/ui/* must echo it in X-RP-CSRF and match
// the cookie (constant-time). An attacker on another origin can
// neither set nor read our cookie, so they can't satisfy both halves.
...
export const csrfIssueHandler = createMiddleware<HonoApp>(async (c) => {
  const cookieName = c.var.env.EVENTS_CSRF_COOKIE_NAME
...
export function requireCsrf() {
  return createMiddleware<HonoApp>(async (c, next) => {
    ...
    const cookieValue = readCookie(c.req.header('cookie') ?? '', c.var.env.EVENTS_CSRF_COOKIE_NAME)
