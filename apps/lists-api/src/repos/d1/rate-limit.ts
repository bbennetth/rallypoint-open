...
// D1 rate-limit impl. Atomic increment of the current window via
// INSERT ... ON CONFLICT ... DO UPDATE + RETURNING. The previous
// window is a plain SELECT (eventually consistent — overcounting
// is fine, undercounting is the worry).
...
export class D1RateLimitRepo implements RateLimitRepo {
  ...
  async takeToken(input: TakeTokenInput): Promise<RateLimitDecision> {
    ...
    // Opportunistic pruning (no cron — lists-api has no scheduled handler):
    // on the first hit of a new window for this bucket (currentCount === 1),
    // drop the bucket's windows older than the previous one. They can never
    // contribute to a future sliding-window blend, so this caps each actively
    // used bucket at its two live windows and bounds the table over time
    // without a scheduler. Indexed by the (tenant, bucket, window) PK prefix;
    // fires at most once per window per bucket.
    if (currentCount === 1) {
      await this.db
        .delete(rateLimits)
        .where(
