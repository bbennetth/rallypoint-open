...
// CSRF double-submit, ported from apps/money-api. GET /api/v1/ui/csrf
...
export const csrfIssueHandler = createMiddleware<HonoApp>(async (c) => {
  const cookieName = c.var.env.PLANNER_CSRF_COOKIE_NAME
...
export function requireCsrf() {
  return createMiddleware<HonoApp>(async (c, next) => {
    ...
    const cookieValue = readCookie(c.req.header('cookie') ?? '', c.var.env.PLANNER_CSRF_COOKIE_NAME)
