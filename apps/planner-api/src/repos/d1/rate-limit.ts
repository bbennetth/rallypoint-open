...
// this stays the shared D1 repo (R2 dedup). planner-api is deliberately
// cron-free for domain logic (BFF constraint), so the D1 branch keeps
// pruning opportunistically on window rollover (opportunisticPrune: true);
// the DO backend self-cleans via per-bucket alarms instead.
