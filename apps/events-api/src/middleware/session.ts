...
// events-side session resolution (design §3.13). The cookie carries
// the opaque events bearer; we store sha256(bearer) as the row PK
// and the RPID bearer AES-GCM-sealed alongside. On every request we:
//
...
//     DELETE the row + clear the cookie + 401. A revoked RPID
//     session must not leave a live events session behind.
//   - RPID unreachable (transport error) → 503, and DO NOT delete
//     the row. A transient RPID hiccup is not a revocation; nuking
//     sessions on a blip would sign everyone out on every outage.
//
// We return the 401 responses directly (rather than throwing) so the
// Set-Cookie that clears the cookie survives — the error handler
// builds a fresh Response and would drop headers set on a throw.

export const EVENTS_SESSION_BEARER_PREFIX = 'rpe_sess_'
...
export function requireSession(): MiddlewareHandler<HonoApp> {
  return async (c, next) => {
    const cookieName = c.var.env.EVENTS_SESSION_COOKIE_NAME
    const raw = readCookie(c.req.header('cookie') ?? '', cookieName)
    if (!raw) {
      throw errors.unauthorized()
    }
    if (!tokenHasPrefix(raw, EVENTS_SESSION_BEARER_PREFIX)) {
    ...
    try {
      bearer = decryptBearer({
        ...
        aad: idHash,
        env: { EVENTS_SESSION_KEY_V1: c.var.env.EVENTS_SESSION_KEY_V1 },
      })
    } catch {
