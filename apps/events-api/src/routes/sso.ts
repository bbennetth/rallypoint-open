...
import { EVENTS_SESSION_BEARER_PREFIX, requireSession } from '../middleware/session.js'
import { csrfIssueHandler } from '../middleware/csrf.js'
import { rateLimit } from '../middleware/rate-limit.js'
import { readJsonBody } from './_body.js'

// SSO + session-lifecycle routes (events side of the §3.13
// bootstrap). All live under /api/v1/ui/* so origin + CSRF
// middleware front them; none require an existing session.
...
export const ssoRoutes = new Hono<HonoApp>()
  ...
  .post(
    ...
    // Verify the state nonce against the cookie the web client set
    // before redirecting to RPID (anti-CSRF for the SSO leg). Clear
    // the state cookie either way — it is single-use.
    const stateCookieName = env.EVENTS_SSO_STATE_COOKIE_NAME
    ...
    // Mint the events bearer, seal the RPID bearer AAD-bound to the
    // new row's id_hash, and persist the session.
    const eventsBearer = generateRawToken(EVENTS_SESSION_BEARER_PREFIX)
    const idHash = hashToken(eventsBearer)
    ...
    await c.var.repos.sessions.create({
      ...
      ipHash: hashIp(
        ...
        dailySalt(env.EVENTS_SESSION_KEY_V1),
      ),
    ...
    c.header(
      'Set-Cookie',
      buildSetCookie(env.EVENTS_SESSION_COOKIE_NAME, eventsBearer, {
        maxAge,
        httpOnly: true,
  ...
  .post('/api/v1/ui/signout', async (c) => {
    const env = c.var.env
    const raw = readCookie(c.req.header('cookie') ?? '', env.EVENTS_SESSION_COOKIE_NAME)
    if (raw) {
    ...
    c.header('Set-Cookie', buildClearCookie(env.EVENTS_SESSION_COOKIE_NAME, true, env.NODE_ENV === 'production'))
    return c.body(null, 204)
  })
