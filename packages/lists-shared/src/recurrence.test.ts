...
describe('CreateSeriesSchema', () => {
  ...
  // Idempotency-key field (offline create-retry dedup, mirrors
  // CreateListItemSchema.ref).
  it('accepts a ref and omits it when not supplied', () => {
    expect(CreateSeriesSchema.parse({ ...base, ref: 'tmp_abc123' }).ref).toBe('tmp_abc123')
    expect(CreateSeriesSchema.parse(base).ref).toBeUndefined()
  })

  it('rejects a ref over 256 characters', () => {
    const r = CreateSeriesSchema.safeParse({ ...base, ref: 'x'.repeat(257) })
    expect(r.success).toBe(false)
  })
})
...
// Floating-due resolution — moved here from planner-api's recurrence-time.test.ts
// so it sits with its producer (occurrenceDueDate). The generic resolver needs
// only { seriesId, dueDate }, so a minimal item shape stands in for the DTO.
interface FloatingItem {
  id: string
  seriesId: string | null
  dueDate: string | null
}
function item(over: Partial<FloatingItem> & { id: string }): FloatingItem {
  return { seriesId: null, dueDate: null, ...over }
}
...
describe('resolveRecurrenceDues', () => {
  ...
  it('resolves a series-backed item but leaves a one-off item untouched', () => {
    const out = resolveRecurrenceDues(
      [
        item({ id: 'chore', seriesId: 'lse_1', dueDate: '2026-06-15T10:30:00.000Z' }),
        // One-off due: a genuine absolute instant (client-anchored to local
        // midnight) — must NOT be reinterpreted.
        item({ id: 'one-off', seriesId: null, dueDate: '2026-06-15T07:00:00.000Z' }),
      ],
      tz,
    )
  ...
  it('leaves a series item with a null due untouched', () => {
    const out = resolveRecurrenceDues([item({ id: 'no-due', seriesId: 'lse_1', dueDate: null })], tz)
    expect(out[0]?.dueDate).toBeNull()
  })

  it('passes a series item with a non-floating due through unchanged (defensive)', () => {
    const out = resolveRecurrenceDues([item({ id: 'weird', seriesId: 'lse_1', dueDate: 'not-an-iso' })], tz)
    expect(out[0]?.dueDate).toBe('not-an-iso')
  })

  it('does not mutate the input array or its items', () => {
    const input = [item({ id: 'chore', seriesId: 'lse_1', dueDate: '2026-06-15T10:30:00.000Z' })]
