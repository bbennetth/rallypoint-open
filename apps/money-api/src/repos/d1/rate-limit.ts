...
import { rateLimits } from '@rallypoint/money-db'
import {
  computeBlend,
  windowStartMs,
...
// D1 rate-limit repo for money-api. Mirrors apps/id-api/src/repos/d1/rate-limit.ts.
...
// overcounting is fine, undercounting is the concern).
...
export class D1RateLimitRepo implements RateLimitRepo {
  ...
  async takeToken(input: TakeTokenInput): Promise<RateLimitDecision> {
    ...
    // Opportunistic pruning (no cron — money-api has no scheduled handler):
