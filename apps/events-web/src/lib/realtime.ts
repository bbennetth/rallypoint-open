// Browser-side realtime: hold a WebSocket to a group or event channel and
// treat every frame as a "something changed" signal (the page refetches;
// payloads are pointers, not rows). Phase 4 replaced the SSE/EventSource
// transport with Durable-Objects WebSockets (mirrors
// apps/lists-web/src/lib/realtime.ts).
//
// Connection lifecycle (WebSocket has no built-in reconnect or auth):
...
//      stream down within ~(token TTL + the DO sweep interval).
//   4. On any unclean close, reconnect with exponential backoff and fire
//      onReconnect so the page reconciles anything missed.
//
// Connection-status integration: the first connect/open drives
// useConnectionStore.setSynced(true); disconnect/close drives setSynced(false),
// ref-counted so multiple concurrent subscriptions stay "synced" until the
// last one closes.
//
...
function markClosed(): void {
  openSubscriptionCount = Math.max(0, openSubscriptionCount - 1)
...
function openChannel(tokenPath: string, handlers: StreamHandlers): Unsubscribe {
  ...
  // Per-socket open count for the connection-store: same idempotency guard
  // as the old SSE impl — a socket can fire onclose more than once.
  ...
  const markSocketOpened = (): void => {
    if (countedOpen) return
    countedOpen = true
    markOpened()
  }

  const markSocketClosed = (): void => {
    if (!countedOpen) return
    countedOpen = false
    markClosed()
  }
...
export function subscribeGroupStream(groupId: string, handlers: StreamHandlers): Unsubscribe {
  return openChannel(
    `/api/v1/ui/groups/${encodeURIComponent(groupId)}/realtime-token`,
    handlers,
  )
}

// Event-scoped stream: the server publishes all event-level mutations
// (events header, lineup, map) on a single event channel (Phase 4 collapse),
// so one subscription covers the event detail, lineup, and map views.
export function subscribeEventStream(eventId: string, handlers: StreamHandlers): Unsubscribe {
  return openChannel(
    `/api/v1/ui/events/${encodeURIComponent(eventId)}/realtime-token`,
    handlers,
  )
}
