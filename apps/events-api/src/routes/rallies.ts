...
// loadGroupForAction: reads need 'member', creation is any member's
// (attendee-map "rally your crew" — relaxed from sidekick+ with the Map
// tab), patch/delete stay 'sidekick', and RSVP is a per-member
// self-action ('member'). The :rallyId must belong to the addressed
// group or we 404 (no cross-group leak).
//
// Every mutation publishes a pointer envelope on the group channel so
// other members' Rallies tab and map rally pins refetch live.
...
function serializeRally(
  ...
  return {
    ...
    lng: rally.lng,
    pin:
      rally.pinLayer !== null && rally.pinXPct !== null && rally.pinYPct !== null
        ? { layer: rally.pinLayer, x_pct: rally.pinXPct, y_pct: rally.pinYPct }
        : null,
    status: rally.status,
...
// Fire-and-forget pointer envelope on the group channel. authorId
// suppresses the actor's own echo (mirrors chat.ts / maps.ts).
function publishRally(
  c: Parameters<typeof loadGroupForAction>[0],
  groupId: string,
  operation: 'create' | 'update' | 'delete',
  id: string,
): void {
  publish(c, groupChannel(groupId), envelope('rallies', operation, id, c.var.session!.userId))
}
...
export const ralliesRoutes = new Hono<HonoApp>()
  ...
  .post('/api/v1/ui/groups/:id/rallies', async (c) => {
    ...
    const rally = await c.var.repos.rallies.create({
      ...
      lat: num(body.lat),
      lng: num(body.lng),
      pinLayer: body.pinLayer ?? null,
      pinXPct: body.pinXPct ?? null,
      pinYPct: body.pinYPct ?? null,
    ...
    publishRally(c, group.id, 'create', rally.id)
  ...
  .patch('/api/v1/ui/groups/:id/rallies/:rallyId', async (c) => {
    ...
    const updated = await c.var.repos.rallies.patch(rally.id, {
      ...
      ...(fields.pinLayer !== undefined ? { pinLayer: fields.pinLayer } : {}),
      ...(fields.pinXPct !== undefined ? { pinXPct: fields.pinXPct } : {}),
      ...(fields.pinYPct !== undefined ? { pinYPct: fields.pinYPct } : {}),
    ...
    publishRally(c, group.id, 'update', rally.id)
  ...
  .delete('/api/v1/ui/groups/:id/rallies/:rallyId', async (c) => {
    ...
    publishRally(c, group.id, 'delete', rally.id)
