...
// subject is always the verified session user (never client-supplied), and
// the namespace is restricted to this app's own client ('admin') or the
// shared cross-app bag — RPID enforces the same rule against the caller,
// this is defense-in-depth + a clean local 403. Backs the theme persister
// in admin-web (shared bag only in practice).

const ADMIN_CLIENT = 'admin'

function assertNamespaceAllowed(namespace: string): void {
  if (namespace === SHARED_SETTINGS_NAMESPACE || namespace === ADMIN_CLIENT) return
  throw errors.forbidden('Namespace is not accessible from this app.')
}

// Translate an id-client SettingsError into the admin error envelope so
