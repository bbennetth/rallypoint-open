...
// sessions — money-side session store (mirrors the events/lists sessions
// table; see events-v1 design doc §3.13). id_hash is SHA-256 of the
// money session bearer handed to money-web (the `__Host-rpm_session`
// cookie value). The RPID session bearer that money-api replays to
...
// MONEY_SESSION_KEY_V<n>). AAD binds the ciphertext to id_hash so a
...
// is unchanged). Mirrors packages/events-db/src/schema/sessions.ts and
...
export const sessions = sqliteTable(
  ...
  (t) => ({
    userIdx: index('money_sessions_user_idx').on(t.userId),
    expiresIdx: index('money_sessions_expires_idx').on(t.absoluteExpiresAt),
  }),
)

export type DbMoneySession = typeof sessions.$inferSelect
export type DbMoneySessionInsert = typeof sessions.$inferInsert
